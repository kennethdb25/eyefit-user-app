...
function App() {
  ...
  const LoginValidation = async () => {
    if (localStorage.getItem("accountUserToken")) {
      ...
      const data = await fetch("https://eyefit-shop-800355ab3f46.herokuapp.com:5000/api/users/validate", {
        method: "GET",
        headers: {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(
        `https://eyefit-shop-800355ab3f46.herokuapp.com:5000/api/user/get-checkout?userId=${loginData?.body?._id}`
      );
